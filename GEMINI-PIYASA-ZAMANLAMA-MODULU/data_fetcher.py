...
def fetch_spy_put_call_ratio():
    """
    CBOE Toplam Put/Call Oranı (Total PCR) döndürür.
    SPY opsiyon zincirinden hesaplanan değer (OI tabanlı) gerçek
    CBOE Total PCR'dan çok farklı çıktığı için bu fonksiyon artık
    yalnızca tarafsız bir varsayılan döndürür.

    Doğru değer için: https://www.cboe.com/us/options/market_statistics/daily/
    → "Total Put/Call Ratio" satırına bakın ve sidebar'a girin.
    30 günlük ortalama yaklaşık 0.90-1.15 bandındadır.
    """
    # CBOE API erişilemez durumdayken tarafsız varsayılan (1.0 = nötr)
    return 1.0, None, "CBOE Total (manuel giriş)", None
