...
# ── Paylaşılan parametre sistemi ──────────────────────────────────────────
_PARAMS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".last_params.json")
...
_mkt_list = list(MARKETS.keys())
_mkt_idx = _mkt_list.index(_P["selected_market"]) if _P.get("selected_market") in _mkt_list else 0
selected_market = st.sidebar.radio("Piyasa Seçimi", _mkt_list, index=_mkt_idx)
_save_params(selected_market=selected_market)
market_info = MARKETS[selected_market]

if selected_market == "ABD Borsaları (USA)":
    _usa_idx_list = list(USA_INDEX_OPTIONS.keys())
    _usa_idx_default = _P.get("usa_index", "SPY")
    _usa_idx_default = _usa_idx_default if _usa_idx_default in _usa_idx_list else "SPY"
    usa_index = st.sidebar.selectbox(
        "Endeks", _usa_idx_list,
        index=_usa_idx_list.index(_usa_idx_default),
        format_func=lambda x: f"{x} — {USA_INDEX_OPTIONS[x]}",
    )
    _save_params(usa_index=usa_index)
    market_info = {**market_info, "symbol": usa_index, "display": USA_INDEX_OPTIONS[usa_index]}

st.sidebar.markdown("---")
_rp_list = list(RISK_PROFILES.keys())
_rp_idx = _rp_list.index(_P["risk_profile"]) if _P.get("risk_profile") in _rp_list else 1
...
_save_params(risk_profile=risk_profile)
...
if selected_market == "Borsa İstanbul (BIST)":
    ...
    policy_rate = st.sidebar.number_input("Güncel Politika Faizi (%)", value=float(_P.get("policy_rate", rate_default)), min_value=0.0, max_value=200.0, step=0.25, format="%.2f")
    _save_params(policy_rate=policy_rate)
    annual_inflation = st.sidebar.number_input("Yıllık Enflasyon (%)", value=float(_P.get("annual_inflation", infl_default)), min_value=0.0, max_value=500.0, step=0.1, format="%.2f")
    _save_params(annual_inflation=annual_inflation)
    ...
    cds_value = st.sidebar.number_input("5Y CDS Primi (bps)", value=int(_P.get("cds_value", cds_default)), min_value=0, max_value=2000, step=5)
    _save_params(cds_value=cds_value)
    ...
    yabanci_oran = st.sidebar.number_input("Yabancı Takas Payı (%)", value=float(_P.get("yabanci_oran", 30.0)), min_value=0.0, max_value=100.0, step=0.5, format="%.1f")
    _save_params(yabanci_oran=yabanci_oran)
    yabanci_onceki = st.sidebar.number_input("1 Ay Önceki Yabancı Payı (%)", value=float(_P.get("yabanci_onceki", 30.0)), min_value=0.0, max_value=100.0, step=0.5, format="%.1f")
    _save_params(yabanci_onceki=yabanci_onceki)
...
if selected_market == "ABD Borsaları (USA)":
    ...
    st.sidebar.caption(
        "Gerçek değer için → [cboe.com/market\_statistics/daily]"
        "(https://www.cboe.com/us/options/market_statistics/daily/) "
        "· **Total Put/Call Ratio** satırı  ·  30G ort ≈ 0.90–1.15"
    )
    _default_pcr = float(_P.get("pc_ratio_val", 1.0))
    # session_state JSON değeriyle senkronize et — ilk bağlanmada veya değer değiştiğinde
    if "pcr_widget" not in st.session_state or abs(st.session_state.get("pcr_widget", _default_pcr) - _default_pcr) > 0.005:
        st.session_state["pcr_widget"] = _default_pcr
    pc_ratio_val = st.sidebar.number_input(
        "Total P/C Oranı (CBOE)",
        key="pcr_widget",
        min_value=0.0, max_value=5.0, step=0.01, format="%.2f",
        help="CBOE Toplam Put/Call Oranı — SPY opsiyon zincirinden değil, "
             "CBOE sitesindeki \'Total Put/Call Ratio\' değerini girin. "
             "Günlük değer 0.7-1.5, 30 günlük ortalama yaklaşık 1.0 civarındadır."
    )
    _save_params(pc_ratio_val=pc_ratio_val)
    _, auto_pcr_vol, pcr_expiry, pcr_details = fetch_spy_put_call_ratio()
...
with hacim_cols[1]:
    ...
    obv_color_hex = "#10B981" if obv_trend_pozitif else ("#EF4444" if obv_trend_pozitif is not None else "#6B7280")
    st.markdown(f"""
        <div style="background:rgba(255,255,255,0.03);border-radius:8px;padding:8px 10px;">
            <div style="font-size:0.75rem;color:#9CA3AF;margin-bottom:4px;">OBV 5G Trend</div>
            <div style="font-size:0.82rem;font-weight:600;color:{obv_color_hex};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{obv_trend_label}</div>
...
if selected_market == "Borsa İstanbul (BIST)":
    ...
    with cds_col1:
        ...
        st.plotly_chart(cds_gauge, use_container_width=True, config={"displayModeBar": False})
    ...
    with cds_col2:
        ...
        if bist_usd is not None and not bist_usd.empty:
            ...
            st.plotly_chart(bist_usd_fig, use_container_width=True, config={"displayModeBar": False})
...
if selected_market == "ABD Borsaları (USA)":
    if vix_current is not None and vix_hist is not None and not vix_hist.empty:
        ...
        st.plotly_chart(vix_fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with usa_col1:
        ...
        if pc_ratio_val is not None:
            ...
            st.plotly_chart(pc_gauge, use_container_width=True, config={"displayModeBar": False})
    ...
    with usa_col2:
        ...
        if dxy_hist is not None and not dxy_hist.empty:
            ...
            st.plotly_chart(dxy_fig, use_container_width=True, config={"displayModeBar": False})
