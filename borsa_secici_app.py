...
st.set_page_config(page_title="Borsa Portföy Seçici & Backtest", layout="wide")
...
def fetch_sp900_tickers() -> list:
    """Wikipedia'dan S&P 500 + MidCap 400 listesini çeker (~900 hisse). Hata olursa fallback kullanılır."""
    import urllib.request, re as _re
    tickers = set()
    urls = [
    ...
    for url in urls:
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                html = resp.read().decode("utf-8", errors="ignore")
            found = _re.findall(r'<td[^>]*>\s*<a[^>]*>([A-Z]{1,5})</a>\s*</td>', html)
            tickers.update(t for t in found if t and len(t) <= 5)
        except Exception:
            continue
    result = sorted(list(tickers))
    if len(result) >= 500:
        return result
    return sorted(list(set(_SP500_FALLBACK + _SP400_FALLBACK)))
...
def get_us_stock_pool(pool_name: str) -> tuple:
    ...
    elif pool_name == "S&P 500 + MidCap 400":
    ...
    for period_idx in range(_regime_total):
        ...
        for strat_name in ["Alfa Portföyü", "Beta Portföyü", "Delta Portföyü"]:
            ...
            if strat_name == "Alfa Portföyü" and scored_stocks:
                ...
                for s in scored_stocks:
                    rs_norm = (s["rs_slope"] / max_rs * 100) if max_rs != 0 else 50
                    score = rs_norm * (1 - s.get("vol_penalty", 0))
                    s["Yatırım Uzmanı Skoru"] = round(max(0, min(100, score)), 1)
...
st.title("Borsa Portföy Seçici & Backtest")
...
with st.sidebar:
    ...
    bt_top_n_weight = st.selectbox("Pozisyon Ağırlıklandırma", ["Sıralama Ağırlıklı", "Eşit Ağırlık"], key="bt_weight_type")
    bt_weighting = "rank" if bt_top_n_weight == "Sıralama Ağırlıklı" else "equal"
    bt_hold_continuation = st.checkbox("Güçlü Pozisyonları Tut (Hold Continuation)", value=True, key="bt_hold_cont")
...
```
    ...
    if bt_result:
        ...
        if bt_result["trade_log"]:
            ...
            st.dataframe(filtered_trade_df, use_container_width=True, hide_index=True)
