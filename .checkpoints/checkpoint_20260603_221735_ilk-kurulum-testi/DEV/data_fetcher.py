...
def get_provider() -> Optional[PriceProvider]:
    """Twelve Data aboneliği iptal edildi. Her zaman None döner."""
    return None
...
def _get_fmp_provider():
    ...
    if _fmp_provider_instance is None:
        try:
            ...
            logger.warning(f"FMP provider yüklenemedi: {e}")
...
def fetch_price_history(
    ...
    # USA hisseleri: FMP (hızlı, güvenilir)
    if not ticker.upper().endswith(".IS"):
        fmp = _get_fmp_provider()
        if fmp is not None:
            try:
                df = fmp.get_daily_history(ticker, outputsize=outputsize, market=market)
                if not df.empty and len(df) >= 20:
                    return df
            except Exception as e:
                logger.warning(f"FMP fetch_price_history hatası {ticker}: {e}")

    # BIST hisseleri (ve FMP başarısız USA): yfinance
...
def fetch_latest_price(ticker: str, market: Optional[str] = None) -> Optional[float]:
    ...
    if not ticker.upper().endswith(".IS"):
        ...
        if fmp is not None:
            try:
                price = fmp.get_latest_price(ticker, market=market)
                if price is not None:
                    return price
            except Exception:
                pass
    # BIST (ve FMP başarısız USA): yfinance
    try:
        from yahoo_provider import fetch_latest_price_yahoo
        return fetch_latest_price_yahoo(ticker)
    except Exception:
        pass
    return None
...
def _enrich_via_yahoo(record: dict) -> dict:
    ...
    # 1. Batch cache
    if ticker in _BATCH_PRICE_CACHE:
        history = _BATCH_PRICE_CACHE[ticker]
    else:
        # 2. Disk cache / Yahoo fallback
        from disk_cache import read_cache, write_cache, needs_refresh
        from symbol_mapper import resolve_twelve_symbol
        try:
            resolved = resolve_twelve_symbol(ticker, market)
            cached = read_cache(resolved)
            if cached is not None and not cached.empty and not needs_refresh(resolved):
                history = cached
            else:
                history = _try_yahoo_fallback(ticker, market=market)
                if not history.empty:
                    write_cache(resolved, history)
...
# Batch price cache - tarama oturumu boyunca geçerli
...
def _fetch_single_ticker(args: tuple) -> tuple:
    ...
    try:
        resolved_sym = resolve_twelve_symbol(symbol, market)  # cache her zaman resolved isimle
...
def fetch_market_data(market: str, skip_fundamentals: bool = False) -> pd.DataFrame:
    ...
    # Toplu fiyat indirme (batch yf.download)
    global _BATCH_PRICE_CACHE
    _BATCH_PRICE_CACHE = {}
    try:
        import yfinance as _yf_b
        from symbol_mapper import resolve_yahoo_symbol as _res
        _ysyms = [_res(s, market) for s in symbols]
        logger.info("Batch fiyat indiriliyor: %d hisse...", len(_ysyms))
        _raw = _yf_b.download(_ysyms, period="2y", interval="1d",
                               auto_adjust=True, progress=False,
                               threads=True, group_by="ticker")
        for _ys, _os in zip(_ysyms, symbols):
            try:
                _df = _raw.copy() if len(_ysyms) == 1 else (
                    _raw[_ys].copy() if _ys in _raw.columns.get_level_values(0) else None)
                if _df is None or _df.empty:
                    continue
                _df.columns = [c.lower() for c in _df.columns]
                if "adj close" in _df.columns:
                    _df = _df.rename(columns={"adj close": "close"})
                _df.index = pd.to_datetime(_df.index)
                _df = _df.dropna(subset=["close"])
                _BATCH_PRICE_CACHE[_os] = _df
            except Exception:
                pass
        logger.info("Batch tamamlandı: %d/%d hisse", len(_BATCH_PRICE_CACHE), len(symbols))
    except Exception as _be:
        logger.warning("Batch indirme başarısız (%s) — tek tek indirilecek", _be)
...
def refresh_eod_cache(market: str, progress_callback=None) -> Dict[str, int]:
    ...
    def _refresh_one(symbol: str) -> tuple:
        resolved = resolve_twelve_symbol(symbol, market)  # cache her zaman resolved isimle
        try:
...
def fetch_backtest_data(
    ...
    from disk_cache import read_cache, write_cache
    ...
    start_time = time.time()
    stats = DataPrepStats()
    all_symbols = SUPPORTED_MARKETS[market]["symbols"]

    # Universe filtresi: sadece seçili evrendeki hisseleri yükle (UX + performans)
    if universe and market == "BIST" and universe != "BISTTUM":
        from config import BIST100_TICKERS
        if universe == "BIST100":
            symbols = [s for s in all_symbols if s in set(BIST100_TICKERS)]
        elif universe == "BIST100_DISI":
            bist100_set = set(BIST100_TICKERS)
            symbols = [s for s in all_symbols if s not in bist100_set]
        else:
            symbols = all_symbols
    elif universe and market == "USA" and universe != "USA_ALL":
        from config import SP500_TICKERS, MIDCAP400_TICKERS
        if universe == "SP500":
            sp500_set = set(SP500_TICKERS)
            symbols = [s for s in all_symbols if s in sp500_set]
        elif universe == "MIDCAP400":
            mid_set = set(MIDCAP400_TICKERS)
            symbols = [s for s in all_symbols if s in mid_set]
        else:
            symbols = all_symbols
    else:
        symbols = all_symbols
    ...
    for idx, symbol in enumerate(symbols):
        ...
        try:
            resolved = resolve_twelve_symbol(symbol, market)  # cache her zaman resolved isimle saklanır

            # Cache'i kontrol et ve gerekirse yenile (stale data fix)
            from disk_cache import get_cached_or_fetch, get_missing_date_range
            cached_raw = read_cache(resolved)
            if cached_raw is not None and not cached_raw.empty:
                # Cache var ama stale olabilir — son tarih bugünden geriyse yenile
                try:
                    missing_start = get_missing_date_range(resolved)
                    if missing_start:
                        # Eksik veriyi yfinance/FMP'den taze çek
                        logger.info("Backtest: refreshing stale cache for %s from %s", symbol, missing_start)
                        def _make_fetch_fn(sym, mkt):
                            def _fn(s, start_date=None):
                                try:
                                    return fetch_price_history(sym, outputsize=500, market=mkt)
                                except Exception:
                                    return None
                            return _fn
                        fresh = get_cached_or_fetch(resolved, _make_fetch_fn(symbol, market), outputsize=500)
                        cached = fresh if (fresh is not None and not fresh.empty) else cached_raw
                    else:
                        cached = cached_raw
                except Exception as _e:
                    logger.warning("Backtest cache refresh failed for %s: %s — using stale data", symbol, _e)
                    cached = cached_raw
            else:
                cached = cached_raw
            ...
            if cached is not None and not cached.empty:
                ...
                # Cache yok — Yahoo'dan direkt çek ve cache'e yaz
                logger.info("Backtest: no cache for %s — fetching from Yahoo directly", symbol)
                try:
                    from yahoo_provider import fetch_yahoo_history
                    fresh_df = fetch_yahoo_history(symbol, period="2y", market=market)
                    if fresh_df is not None and not fresh_df.empty:
                        write_cache(resolved, fresh_df)
                        price_data = _ensure_sorted(fresh_df)
                        price_data = _filter_price_spikes(price_data)
                        stats.full_fetches += 1
                    else:
                        logger.warning("Backtest: Yahoo returned empty data for %s — skipping", symbol)
                        stats.failed += 1
                        stats.failed_symbols.append(symbol)
                        continue
                except Exception as _fetch_e:
                    logger.error("Backtest: direct fetch failed for %s: %s", symbol, _fetch_e)
                    stats.failed += 1
                    stats.failed_symbols.append(symbol)
                    continue
