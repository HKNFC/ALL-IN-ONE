...
def compute_rs_scores(df: pd.DataFrame, market: str = None,
    ...
    # BIST: Temel veri büyük çoğunlukla NaN ise (backtest) —
    # saf momentum/RS skorunu rs_score olarak kullan (Mark Minervini yaklaşımı)
    if _is_bist:
        ...
        _available = sum(
            1 for c in _fin_cols
            if c in result.columns and result[c].notna().sum() > len(result) * 0.30
        )
        if _available < 2:
            # Fundamental datanın %70'inden fazlası eksik → momentum skoru kullan
