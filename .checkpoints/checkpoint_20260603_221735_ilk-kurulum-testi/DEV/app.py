...
with tab_backtest:
    ...
    if bt_start >= bt_end:
        ...
        # Cache tazelik kontrolü
        try:
            from disk_cache import CACHE_DIR
            import os, glob as _glob
            _parquet_files = _glob.glob(os.path.join(CACHE_DIR, "*.parquet"))
            if _parquet_files:
                _latest_mtime = max(os.path.getmtime(f) for f in _parquet_files)
                _cache_last_date = pd.Timestamp(_latest_mtime, unit="s").date()
            else:
                _cache_last_date = None
        except Exception:
            _cache_last_date = None

        _bt_end_date = bt_end
        _cache_stale = _cache_last_date is not None and (date.today() - _cache_last_date).days > 5

        if _cache_stale:
            st.warning(
                f"⚠️ Cache verisi {_cache_last_date} tarihinde güncellendi. "
                f"Bugünkü veriler için önce aşağıdaki butona basın."
            )
            if st.button("Veriyi Güncelle (Cache Yenile)", key="bt_refresh_cache"):
                with st.spinner(f"{bt_market} verileri güncelleniyor, bu birkaç dakika sürebilir..."):
                    try:
                        refresh_eod_cache(bt_market)
                        st.success("Veriler güncellendi! Şimdi backtest çalıştırabilirsiniz.")
                        st.rerun()
        ...
        if run_bt:
            # Cache bitiş tarihe yetişmiyorsa otomatik yenile
            if _cache_last_date is not None and bt_end > _cache_last_date:
                with st.spinner(f"{bt_market} cache yenileniyor ({_cache_last_date} → {bt_end})..."):
                    try:
                        refresh_eod_cache(bt_market)
                    except Exception as _e:
                        st.warning(f"Cache yenilenirken hata ({_e}); mevcut cache ile devam ediliyor.")
