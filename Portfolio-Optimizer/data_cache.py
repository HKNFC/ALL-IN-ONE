...
# ── SQLite DB katmanı ─────────────────────────────────────────────────────────
_DB_PATH = os.path.join(os.path.dirname(__file__), "..", "MARK MİNERVİNİ", "data", "stock_prices.db")
_DB_PATH = os.path.normpath(_DB_PATH)

def _get_db():
    """StockDB bağlantısı — DB yoksa None döndür."""
    try:
...
def _df_from_db(ticker: str, start_str: str, end_str: str) -> pd.DataFrame:
    """DB'den OHLCV verisi çek, _normalize_df ile uyumlu format döndür."""
    db = _get_db()
    if db is None:
        return pd.DataFrame()
    try:
        df = db.get_prices(ticker, start_str, end_str)
...
def _save_to_db(ticker: str, df: pd.DataFrame):
    """Verilen df'i DB'ye yaz (upsert)."""
    db = _get_db()
...
def get_price_data(ticker: str, start_str: str, end_str: str) -> pd.DataFrame:
    """Tek hisse OHLCV: pkl cache → SQLite DB → Twelve Data/Yahoo."""
    ...
    # 2. SQLite DB
    df = _df_from_db(ticker, start_str, end_str)
    if not df.empty:
        logger.debug("DB'den yüklendi: %s", ticker)
        _write_cache(ticker, df, start_str, end_str)
        return df

    # 3. Twelve Data / Yahoo (fallback)
    if _is_bist(ticker):
...
def batch_get_price_data(tickers: list, start_str: str, end_str: str,
    ...
    if missing_bist:
        td_result = _fetch_td_batch(
            missing_bist, start_str, end_str, progress_callback=progress_callback)
        for t, df in td_result.items():
            if not df.empty:
                _save_to_db(t, df)
        result.update(td_result)

    # ABD → yfinance toplu (Twelve Data rate-limit'e takılmaz)
    if missing_us:
        yf_result = _yf_bulk_download(missing_us, start_str, end_str, progress_callback=progress_callback)
