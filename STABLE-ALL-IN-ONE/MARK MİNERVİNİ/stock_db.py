...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "stock_prices.db")
...
class StockDB:
    ...
    def has_data(self, ticker: str, min_rows: int = 50, max_stale_days: int = 1) -> bool:
        """
        Ticker için yeterli veri var mı kontrol eder.
        max_stale_days: canlı taramada DB verisi bu kadar günden eskiyse False döner
                        (yf fallback devreye girer ve DB güncellenir).
        """
        import datetime
        row = _conn().execute(
        ...
        if not (status == "ok" and (row_count or 0) >= min_rows):
            return False
        # Son tarihi kontrol et — eski veri varsa False dön ki yf yeniden çeksin
        try:
            last = _conn().execute(
            ...
            if last is None:
                return False
            last_dt = datetime.date.fromisoformat(str(last)[:10])
            today = datetime.date.today()
            # Hafta sonu/tatil toleransı: takvim günü farkı
            # Hafta sonu toleransı: Cuma EOD = Pazar ve Pazartesi sabahı hala geçerli
            is_weekend_gap = today.weekday() in (5, 6) or (today.weekday() == 0 and (today - last_dt).days <= 3)
            effective_stale = max_stale_days + (2 if is_weekend_gap else 0)
            if (today - last_dt).days > effective_stale:
                return False
        except Exception:
            pass  # tarih kontrolü başarısız olursa mevcut davranışı koru
        return True
