...
from scoring_bist import score_bist_macro, score_bist_health, score_bist_timing, score_bist_money_flow, score_bist_momentum
from scoring_usa import score_usa_risk, score_usa_internals, score_usa_timing, score_usa_momentum, score_usa_sentiment
from decision_engine import decide_bist, decide_usa, get_verdict_style
from backtest import run_backtest
from historical_simulator import run_simulation
...
enable_simulation = st.sidebar.checkbox("📅 Tarih Aralıklı Simülasyon", value=False)
...
if selected_market == "ABD Borsaları (USA)":
    st.sidebar.markdown("**CBOE Toplam Put/Call Oranı**")
    st.sidebar.caption(
        "Gerçek değer için → [cboe.com/market_statistics/daily]"
        "(https://www.cboe.com/us/options/market_statistics/daily/) "
        "· **Total Put/Call Ratio** satırı  ·  30G ort ≈ 0.90–1.15"
    )
    _, auto_pcr_vol, _, _ = fetch_spy_put_call_ratio()
    pc_volume_ratio = auto_pcr_vol

    if "pcr_widget_dash" not in st.session_state:
        st.session_state["pcr_widget_dash"] = 0.90

    pc_ratio_val = st.sidebar.number_input(
        "Total P/C Oranı (CBOE)",
        key="pcr_widget_dash",
        min_value=0.0, max_value=5.0, step=0.01, format="%.2f",
        help="CBOE sitesinden manuel girin: cboe.com/us/options/market_statistics/daily/ → TOTAL PUT/CALL RATIO"
    )
    st.sidebar.caption("📋 [CBOE'dan bak](https://www.cboe.com/us/options/market_statistics/daily/) → Total Put/Call Ratio")
...
if selected_market == "Borsa İstanbul (BIST)":
    ...
    money_flow_score, money_flow_indicators, money_flow_max = score_bist_money_flow(
        hist, usdtry_hist=usdtry_hist, cds_value=cds_value, cds_trend=cds_trend,
        bist_usd_above=bist_usd_above_sma200,
        yabanci_oran=yabanci_oran, yabanci_degisim=yabanci_degisim,
    )
    momentum_score, momentum_indicators, momentum_max = score_bist_momentum(hist)
    ...
    # ── Kredi Spread / Yield Curve / Likidite ───────────────────────────────
    credit_spread_val = None
    yield_curve_val   = None
    liquidity_val     = None
    try:
        import yfinance as _yf
        _t10 = _yf.Ticker("^TNX").history(period="5d")["Close"]
        _t3m = _yf.Ticker("^IRX").history(period="5d")["Close"]
        _hyg = _yf.Ticker("HYG").history(period="3mo")["Close"]
        _ief = _yf.Ticker("IEF").history(period="3mo")["Close"]
        # Yield Curve: 10Y - 3M (pozitif = normal, negatif = ters)
        if not _t10.empty and not _t3m.empty:
            yield_curve_val = round(float(_t10.iloc[-1]) - float(_t3m.iloc[-1]), 2)
        # Credit Spread proxy: HYG/IEF momentum (HYG güçleniyorsa spread daralıyor = risk-on)
        if len(_hyg) >= 20 and len(_ief) >= 20:
            hyg_ret = float(_hyg.iloc[-1]) / float(_hyg.iloc[-20]) - 1
            ief_ret = float(_ief.iloc[-1]) / float(_ief.iloc[-20]) - 1
            credit_spread_val = 1 if hyg_ret > ief_ret else -1  # +1 daralan, -1 açılan
        # Likidite: HYG 50G SMA üzerinde mi?
        if len(_hyg) >= 50:
            hyg_sma50 = float(_hyg.rolling(50).mean().iloc[-1])
            liquidity_val = 1 if float(_hyg.iloc[-1]) > hyg_sma50 else -1
    except Exception:
        pass
    ...
    momentum_score, momentum_indicators, momentum_max = score_usa_momentum(hist, rs_data)
    sentiment_score, sentiment_indicators, sentiment_max = score_usa_sentiment(
        hist, pc_ratio=pc_ratio_val, pc_volume_ratio=pc_volume_ratio,
    )
...
# Fırsatçı profil için seçici hisse notu
if risk_profile == "Fırsatçı":
    profile_data = RISK_PROFILES.get("Fırsatçı", {})
    selective_notes = profile_data.get("selective_note", {})
    note_text = selective_notes.get(verdict, "")
    if note_text:
        st.markdown(f"""
        <div style="background:#FFF7ED; border:2px solid #F97316; border-radius:10px; padding:14px 20px; margin-top:8px; margin-bottom:16px;">
            <div style="font-weight:700; color:#C2410C; font-size:15px; margin-bottom:4px;">⚡ Fırsatçı Profil — Seçici Giriş İmkânı</div>
            <div style="color:#9A3412; font-size:14px;">{note_text}</div>
        </div>
        """, unsafe_allow_html=True)

if selected_market == "Borsa İstanbul (BIST)":
    sub_labels = [("Makro", macro_score, "🏛️"), ("Para Akışı", money_flow_score, "💰"), ("Piyasa Genişliği", health_score, "🏥"), ("Teknik", timing_score, "⏱️"), ("Momentum", momentum_score, "🚀")]
    all_indicators = [("Makro Göstergeler", macro_indicators), ("Para Akışı", money_flow_indicators), ("Piyasa Genişliği", health_indicators), ("Teknik", timing_indicators), ("Momentum", momentum_indicators)]
else:
    sub_labels = [("Risk Ortamı", risk_score, "🛡️"), ("Piyasa Genişliği", internals_score, "📊"), ("Teknik", timing_score, "⏱️"), ("Momentum", momentum_score, "🚀"), ("Sentiment", sentiment_score, "🧠")]
    all_indicators = [("Risk Ortamı", risk_indicators), ("Piyasa Genişliği", internals_indicators), ("Teknik", timing_indicators), ("Momentum", momentum_indicators), ("Sentiment", sentiment_indicators)]
...
_vol_pos = volume[volume > 0] if volume is not None else None
if _vol_pos is not None and len(_vol_pos) > 0:
    _sma_tmp = _vol_pos.rolling(20).mean().dropna()
    _sma_tmp_val = _sma_tmp.iloc[-1] if len(_sma_tmp) > 0 else None
    last_volume = _vol_pos.iloc[-1]
    # Bugün eksik veri (hafta sonu / piyasa yeni açıldı): SMA'nın %5'inden azsa bir önceki günü al
    if _sma_tmp_val and last_volume < _sma_tmp_val * 0.05 and len(_vol_pos) >= 2:
        last_volume = _vol_pos.iloc[-2]
else:
    last_volume = 0
vol_sma20_val = vol_sma20.dropna().iloc[-1] if vol_sma20 is not None and len(vol_sma20.dropna()) > 0 else None
hacim_onayi = vol_sma20_val is not None and vol_sma20_val > 0 and last_volume > vol_sma20_val
...
with hacim_cols[0]:
    ...
    st.metric("Güncel Hacim / Ort.", f"{vol_ratio:.1f}%",
...
if selected_market == "Borsa İstanbul (BIST)":
    ...
    with cds_col1:
        ...
        st.plotly_chart(cds_gauge, use_container_width=True)
    ...
    with cds_col2:
        ...
        if bist_usd is not None and not bist_usd.empty:
            ...
            st.plotly_chart(bist_usd_fig, use_container_width=True)
...
if selected_market == "ABD Borsaları (USA)":
    ...
    with usa_col1:
        ...
        if pc_ratio_val is not None:
            ...
            st.plotly_chart(pc_gauge, use_container_width=True)
...
if enable_simulation:
    st.markdown("---")
    st.subheader("📅 Tarih Aralıklı Simülasyon")
    
    sim_col1, sim_col2, sim_col3 = st.columns(3)
    with sim_col1:
        sim_start = st.date_input("Başlangıç", value=pd.Timestamp.today() - pd.Timedelta(days=30))
    with sim_col2:
        sim_end = st.date_input("Bitiş", value=pd.Timestamp.today())
    with sim_col3:
        sim_interval = st.selectbox(
            "Periyot",
            ["Tüm Günler", "1 Hafta", "15 Gün", "1 Ay"],
            index=0
        )
    
    if st.button("🚀 Simülasyon Çalıştır", type="primary"):
        with st.spinner("Simülasyon hesaplanıyor..."):
            market_code = "BIST" if selected_market == "Borsa İstanbul (BIST)" else "USA"
            sim_df, sim_err = run_simulation(
                market_code,
        ...
        if sim_err:
            st.error(f"Simülasyon hatası: {sim_err}")
        elif sim_df is None or sim_df.empty:
            st.warning("Simülasyon için yeterli veri yok.")
        else:
            # Periyota göre örnekleme - ayın ilk işlem günü bazlı
            if sim_interval == "Tüm Günler":
                sim_df_display = sim_df.copy()
            else:
                sim_df_display = []
                
                # Başlangıç tarihinden sonraki ilk işlem gününü bul
                start_str = sim_start.strftime("%Y-%m-%d")
                start_idx = sim_df[sim_df["date"] >= start_str].index
                if len(start_idx) == 0:
                    sim_df_display = sim_df.copy()
                else:
                    current_idx = start_idx[0]
                    
                    while current_idx in sim_df.index:
                        sim_df_display.append(sim_df.loc[current_idx])
                        
                        # Bir sonraki ayın ilk işlem gününü bul
                        current_date = pd.Timestamp(sim_df.loc[current_idx, "date"])
                        if sim_interval == "1 Hafta":
                            next_date = current_date + pd.Timedelta(days=7)
                        elif sim_interval == "15 Gün":
                            next_date = current_date + pd.Timedelta(days=15)
                        elif sim_interval == "1 Ay":
                        ...
                        # Bu tarihten sonraki ilk işlem gününü bul
                        future = sim_df[sim_df["date"] >= next_date.strftime("%Y-%m-%d")]
                        if len(future) > 0:
                            current_idx = future.index[0]
                        else:
                            break
                
                sim_df_display = pd.DataFrame(sim_df_display) if sim_df_display else sim_df.copy()
            
            st.success(f"{len(sim_df)} gün için simülasyon tamamlandı")
            
            # Özet metrikler
            sim_metrics = st.columns(4)
            with sim_metrics[0]:
                avg_hisse = sim_df_display["hisse_pct"].mean()
                st.metric("Ort. Hisse", f"%{avg_hisse:.0f}")
            with sim_metrics[1]:
                avg_tahvil = sim_df_display["tahvil_pct"].mean()
                st.metric("Ort. Tahvil", f"%{avg_tahvil:.0f}")
            with sim_metrics[2]:
                avg_nakit = sim_df_display["nakit_pct"].mean()
                st.metric("Ort. Nakit", f"%{avg_nakit:.0f}")
            with sim_metrics[3]:
                uygun_gun = len(sim_df_display[sim_df_display["verdict"].isin(["UYGUN", "AGRESİF ALIM UYGUN", "KADEMELİ ALIM"])])
                st.metric("Alım Uygun Gün", f"{uygun_gun}/{len(sim_df_display)}")
            
            # Tablo
            st.markdown("##### Günlük Kararlar")
            
            # Renkli tablo için
            def color_verdict(val):
                if val in ["UYGUN", "AGRESİF ALIM UYGUN"]:
                    return "color: #10B981; font-weight: bold"
                elif val in ["KADEMELİ ALIM", "KADEMELİ / DİKKATLİ İŞLEM"]:
                    return "color: #F59E0B; font-weight: bold"
                elif val in ["DİKKATLİ", "NÖTR / BEKLE", "BEKLE"]:
                    return "color: #6B7280"
                else:
                    return "color: #EF4444; font-weight: bold"
            
            display_df = sim_df_display[["date", "price", "verdict", "hisse_pct", "tahvil_pct", "nakit_pct", "total_score"]].copy()
            display_df.columns = ["Tarih", "Fiyat", "Karar", "Hisse %", "Tahvil %", "Nakit %", "Skor"]
            
            st.dataframe(
                display_df.style.applymap(color_verdict, subset=["Karar"]),
                use_container_width=True,
                height=400
            )
            
            # CSV indirme
            csv = sim_df_display.to_csv(index=False).encode("utf-8")
            st.download_button(
                "📥 CSV İndir",
                csv,
                f"simulasyon_{market_code}_{sim_start}_{sim_end}.csv",
                "text/csv"
            )
