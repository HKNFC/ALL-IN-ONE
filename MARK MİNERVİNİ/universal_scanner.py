...
# Twelve Data aboneliği iptal edildi — yfinance kullanılıyor
_TD_AVAILABLE = False
...
class UniversalStockScanner:
    ...
    def get_us_tickers(self):
        """FMP S&P500+NASDAQ-100 (birincil), standart evren fallback."""
        try:
    ...
    def get_bist_tickers(self):
        ...
        try:
            ...
            bist_codes = [
                ...
                # Gıda & İçecek
                'AEFES', 'BANVT', 'CCOLA', 'ERSU', 'KNFRT', 'PENGD', 'PETUN',
    ...
    def _get_close_col(self, df):
        """DataFrame'den Close sütununu güvenli şekilde döndür.
        yfinance.download() MultiIndex veya ticker-named sütun döndürebilir."""
        if 'Close' in df.columns:
            return df['Close']
        if 'close' in df.columns:
            return df['close']
        # MultiIndex: ('Close', 'XU100.IS') gibi
        import pandas as pd
        ...
        # Tek ticker download: sütun adı='XU100.IS' gibi — tek sütunsa onu al
        if len(df.columns) == 1:
            return df.iloc[:, 0]
        # Birden fazla sütun ama hiç Close yok → ilk sütunu dene
        return df.iloc[:, 0]
    ...
    def calculate_rs_bist(self, stock_df, xu100_df):
        ...
        try:
            ...
            x = self._get_close_col(xu100_df).loc[common]
    ...
    def scan_us_stock(self, ticker, sp500_data, stock_data=None, as_of_date=None):
        ...
        try:
            ...
            if rs is None:
                return None  # RS hesaplanamazsa hisseyi tarama dışı bırak
    ...
    def _fetch_ohlcv(self, ticker, as_of_date=None):
        ...
        # ── 2. FMP (USA hisseleri için — hızlı, güvenilir) ────────────
        if not ticker.endswith('.IS'):
            try:
                import sys as _sys
                _sys.path.insert(0, '/Users/hakanficicilar/Documents/Aİ')
                from fmp_provider import get_history as _fmp_hist
                fmp_df = _fmp_hist(ticker, from_date=start, to_date=end)
                if fmp_df is not None and len(fmp_df) >= 50:
                    fmp_df.index = pd.to_datetime(fmp_df.index).normalize()
                    return fmp_df[fmp_df.index <= cutoff]
            except Exception:
                pass

        # ── 3. Yahoo Finance (BIST birincil, USA fallback) ──────────────
