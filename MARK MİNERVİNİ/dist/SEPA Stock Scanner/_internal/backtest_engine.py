    ...
    def bulk_download(self, tickers, start, end, label="hisse"):
        ...
        try:
            ...
            td_results = td.get_batch_time_series(
                td_missing, start, end, batch_size=8, sleep_sec=1.2
            )
            still_missing = []
...
class MinerviniBacktest:
    ...
    def _global_prefetch(self, tickers, market):
        ...
        all_tickers = list(dict.fromkeys(tickers + benchmark))  # tekrarsız

        print(f"\n📥 Veri indirme başlıyor: {fetch_start.date()} → {fetch_end.date()}", flush=True)
        print(f"   {len(tickers)} hisse + 3 benchmark", flush=True)

        self._cache.bulk_download(all_tickers, fetch_start, fetch_end, label="hisse")
