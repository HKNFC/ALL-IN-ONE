...
def api_full_scan():
    ...
    try:
        ...
        if scan_date:
            ...
            results = bt.scan_market_at_date(scan_dt, market=market, tickers_override=tickers_override)
...
def api_get_portfolio():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Güncel fiyat çek
            try:
                yahoo_ticker = f"{ticker}.IS" if (str(ticker).isalpha() and len(ticker) <= 6 and not any(c in ticker for c in ['.', '-'])) else ticker
                stock = yf.Ticker(yahoo_ticker)
                hist = stock.history(period='1d')
                current_price = hist['Close'].iloc[-1] if len(hist) > 0 else entry_price
            except:
...
def _sanitize(obj):
    """numpy / pandas tiplerini JSON-safe Python tiplerine çevirir."""
    import numpy as np
    import pandas as pd
...
def api_run_backtest():
    ...
    try:
        ...
        return jsonify({'success': True, 'report': report, 'saved_id': bt_id, 'saved_name': name})
