...
def select_ensemble_portfolio(
    ...
    """Seçili modüllerden n_picks kadar hisse al, örtüşenleri alt sıradan tamamla."""
    # Aktif modüller: (liste, model_adı, kaç_hisse)
    active = []
    if use_mm: active.append((mm_results, "Mark Minervini",   n_picks_mm))
    if use_po: active.append((po_results, "Portföy Optimizer", n_picks_po))
    if use_si: active.append((si_results, "Super Investor",   n_picks_si))

    selected = {}
    max_picks = max((n for _, _, n in active), default=3)

    # Round-robin: sırayla her modülden ilk n_picks hisseyi al
    for rank in range(max_picks):
        for results, model_name, n_picks in active:
            if rank >= n_picks:
                continue
            if rank < len(results):
    ...
    # Örtüşme durumunda alt sıradan tamamla (hedef = toplam seçim sayısı)
    target = sum(n for _, _, n in active)
    for results, model_name, _ in active:
        for rank in range(max_picks, len(results)):
...
def main():
    ...
    with tab_backtest:
        ...
        # Modül & Hisse Sayısı Ayarları
        with st.expander("🧩 Modül & Hisse Sayısı Ayarları", expanded=True):
            mod_col1, mod_col2, mod_col3 = st.columns(3)
            with mod_col1:
                st.markdown("**Mark Minervini**")
                bt_use_mm = st.checkbox("Dahil Et", value=True, key="ens_bt_use_mm")
                bt_n_mm = st.selectbox("Hisse Sayısı", [1, 3, 5], index=1, key="ens_bt_n_mm", disabled=not bt_use_mm)
            with mod_col2:
                st.markdown("**Portföy Optimizer**")
                bt_use_po = st.checkbox("Dahil Et", value=True, key="ens_bt_use_po")
                bt_n_po = st.selectbox("Hisse Sayısı", [1, 3, 5], index=1, key="ens_bt_n_po", disabled=not bt_use_po)
            with mod_col3:
                st.markdown("**Super Investor**")
                bt_use_si = st.checkbox("Dahil Et", value=True, key="ens_bt_use_si")
                bt_n_si = st.selectbox("Hisse Sayısı", [1, 3, 5], index=1, key="ens_bt_n_si", disabled=not bt_use_si)

            if not (bt_use_mm or bt_use_po or bt_use_si):
                st.warning("⚠️ En az bir modül seçilmelidir.")

            total_picks = (bt_n_mm if bt_use_mm else 0) + (bt_n_po if bt_use_po else 0) + (bt_n_si if bt_use_si else 0)
            st.caption(f"Portföy büyüklüğü: **{total_picks} hisse** · Eşit ağırlık %{round(100/total_picks, 1) if total_picks > 0 else 0}")

        period_months = {"Aylık": 1, "2 Aylık": 2, "3 Aylık": 3}[bt_period]
        
        run_bt = st.button("📈 Backtest Başlat", use_container_width=True, disabled=not (bt_use_mm or bt_use_po or bt_use_si))
        ...
        if run_bt:
            ...
            with st.spinner("Backtest çalışıyor..."):
                ...
                for i, rebalance_date in enumerate(dates):
                    ...
                    screen_top = max(bt_n_mm if bt_use_mm else 0,
                                     bt_n_po if bt_use_po else 0,
                                     bt_n_si if bt_use_si else 0) + 5
                    mm_res = screen_minervini_style(tickers, cutoff, bt_market, top_n=screen_top) if bt_use_mm else []
                    po_res = screen_optimizer_style(tickers, cutoff, bt_market, top_n=screen_top) if bt_use_po else []
                    si_res = screen_superinvestor_style(tickers, cutoff, bt_market, top_n=screen_top, quality=bt_si_quality, sort_by=bt_si_sort, profile=bt_si_profile) if bt_use_si else []
                    
                    ensemble = select_ensemble_portfolio(
                        mm_res, po_res, si_res,
                        use_mm=bt_use_mm, use_po=bt_use_po, use_si=bt_use_si,
                        n_picks_mm=bt_n_mm, n_picks_po=bt_n_po, n_picks_si=bt_n_si,
                    )
                    ...
                    row = {"Tarih": cutoff.strftime("%d/%m/%Y")}
                    if bt_use_mm:
                        row[f"Minervini ({bt_n_mm}H)"] = ", ".join([r["ticker"] for r in mm_res[:bt_n_mm]])
                    if bt_use_po:
                        row[f"Optimizer ({bt_n_po}H)"] = ", ".join([r["ticker"] for r in po_res[:bt_n_po]])
                    if bt_use_si:
                        row[f"Sup.Investor ({bt_n_si}H)"] = ", ".join([r["ticker"] for r in si_res[:bt_n_si]])
                    row["Portföy Değeri"] = round(current_value, 0)
                    row["Dönem Getiri %"] = round(avg_return * 100, 2)
                    row["Benchmark %"] = round(bench_ret * 100, 2)
                    trades.append(row)
